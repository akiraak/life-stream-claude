...
#### 進捗 (2026-04-25)
- ✅ ステップ 1: CF Access ポリシー Bypass のまま新コードデプロイ準備
- ✅ ステップ 2: 本番 `.env` 更新（`ADMIN_EMAILS` 削除、`CF_ACCESS_TEAM_DOMAIN=akiraak.cloudflareaccess.com`、`CF_ACCESS_AUD=<本番に投入済>`）
- ✅ ステップ 3: 新コードを g3plus にデプロイ（Phase 2/3/4 のコミットを `git push origin main` → ssh で git pull + `docker compose build/up` → ログで `Server running on http://localhost:3002` 確認）
...
### Phase 4-2 — 管理画面にログアウトボタンを追加（追加実装） ✅ 完了 (2026-04-25)
Phase 4 で `auth_token` 依存を撤去したが、Cloudflare Access のセッション
（`CF_Authorization` Cookie / 24h）を能動的に切るための UI が `web/admin` 側に
無い。SSO ログイン後にユーザーが任意のタイミングで「別の Google アカウントで
入り直したい」「共用端末から離れる前に切りたい」場合の導線が必要。

#### 設計
- 配置: トップバーの email 右側（`web/admin/index.html` の `topbar-user`
  ブロック内）に小さなテキストリンク `ログアウト`
- クリック時: `location.href = '/cdn-cgi/access/logout?returnTo=' + encodeURIComponent('/')`
  - returnTo を `/` にして、ログアウト後はランディング（`/about`/`/`）に戻す
  - returnTo を `/admin/` にすると即座に再ログイン画面に飛んでしまうので避ける
- 401 時の自動ログアウト（`api()` の 401 ハンドラ）は `returnTo=/admin/` のままで
  OK（セッション切れ → 再ログイン誘導が望ましい挙動なので分けて扱う）
- CSS: 既存 `topbar-user` 横に小さく出す。スタイルは既存コンポーネントに揃える

#### 実装範囲
- `web/admin/index.html`: `topbar-user` の隣に `<a id="topbar-logout">` を追加
- `web/admin/app.js`: `initAdmin()` の後段で logout リンクに onclick を bind
  （`location.href = '/cdn-cgi/access/logout?returnTo=/'`）
- `web/admin/style.css`: `.topbar-logout` に小さめの padding / 色 / hover スタイル
- 結合テストは不要（純粋な UI 遷移）

#### 完了条件
- 管理画面右上のメールアドレス横に「ログアウト」リンクが出る
- クリックで `CF_Authorization` Cookie がクリアされ、`/` に着地する
- 再度 `/admin/` を開くと Google SSO のログイン画面が出る

**実装結果:**
- `web/admin/index.html`: `topbar-user` を `<div class="topbar-right">` で
  ラップし、隣に `<a id="topbar-logout">ログアウト</a>` を追加
- `web/admin/app.js`: `initAdmin()` の冒頭で `topbar-logout` の click を
  `location.href = '/cdn-cgi/access/logout?returnTo=' + encodeURIComponent('/')` に bind
- `web/admin/style.css`: `.topbar-right`（flex + gap 12px）、`.topbar-logout`
  （小さめテキスト + 1px ボーダー + hover で背景色変化）を追加
- 本番反映は次回デプロイ時に実機確認（CF Access セッション切断挙動 / `/` 着地）
