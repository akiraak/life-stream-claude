...
- [x] Phase 2: errorHandler 汎用化 + migrate 日本語化
  - [x] 着手時に `Error.status` を付けて throw している箇所を再 grep（無いことを再確認）
  - [x] `errorHandler` を `err.status ?? err.statusCode ?? 500` / `err.expose` 対応に
  - [x] migrate スコープ専用 error middleware で `entity.too.large` を 413 + 日本語化
        （app レベル body-parser 由来エラーを拾うため、router 内ではなく
        `app.use('/api/migrate', bodyParser, migratePayloadTooLargeHandler)` でマウント）
  - [x] `error-handler.test.ts` を「Internal Server Error に塗り潰す」期待に書き換え
  - [x] `migrate.test.ts` に「上限超過は 413 + 日本語エラー」ケースを追加（DI で小さい limit を使う）
